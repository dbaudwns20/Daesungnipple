...
// import { getHierarchyCategoryIdsFromDB } from "@/database/product.category";
//
// /**
//  * 상품 만들기
//  * - 상품 기본 정보 생성
...
//  * @param id
//  * @param searchOption, 삭제된 것도 조회 가능 여부 (default: ALL)
//  * @returns Product
//  */
// export async function getProduct(id: string, searchOption: SearchOption = SearchOption.ALL): Promise<Product | null> {
//   try {
//     return await getProductDB(getSearchOptionAsCondition(id, searchOption)); // 조건에 따라 조회
//   } catch (e: any) {
//     throw new Error(e.message);
//   }
// }
...
//  * @returns Product[]
//  */
// export async function listProduct(opt: ProductListOption): Promise<ListResult> {
//   try {
//     const validatedOption = validateListOption<ProductListOption>(opt);
//     // 카테고리를 검색 키워드로 하는 경우 해당하는 카테고리 id 전부 찾기
//     if (validatedOption.categoryIds && validatedOption.categoryIds.length !== 0)
//       validatedOption.categoryIds = await getHierarchyCategoryIdsFromDB(validatedOption.categoryIds[0]);
//
//     const list = await listProductDB(validatedOption);
//     const totalCount = await countProductDB(validatedOption);
//
