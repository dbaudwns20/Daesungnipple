...
export default function Products() {
  ...
  // 페이지 렌더링시 상품목록 가져오기
  const { data, error, isLoading } = useFetch(`/api/products?${ newSearchParams.toString() }`);
  ...
  return (
    <div className="flex flex-col w-full p-2 space-y-4 bg-blue-200">
      ...
      <div className="grid grid-cols-4 gap-4">
        { products.map((product) => (
          <div key={ product.id }
               className="flex flex-row w-full space-x-2 bg-gray-200">
            <Link href={ `/products/${ product.id }` }>
              <img src={ product.mainImageUrl || dummyImage } className="max-w-[250px] h-[250px]"
                   alt={ "상품 이미지" }></img>
            </Link>
